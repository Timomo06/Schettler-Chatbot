...
const CAMPUS_B27_START_CARDS: StartCard[] = [
  {
    icon: "🚘",
    title: "Ausbildung finden",
    description: "Auto, Motorrad, Lkw oder Bus – zuerst das passende Ziel einordnen",
    action: "hohenbadenPanel",
    hohenbadenPanel: "courses",
  },
  {
    icon: "📅",
    title: "Theoriezeiten",
    description: "Veröffentlichte Termine passend zur Klasse kompakt sehen",
    action: "hohenbadenPanel",
    hohenbadenPanel: "schedule",
  },
  {
    ...
    description: "Demo-Cockpit mit Ziel, Unterlagen und nächstem Schritt",
    action: "hohenbadenPanel",
    hohenbadenPanel: "connect",
  },
  {
...
const REGIONAL_DEMO_CONFIGS: Record<RegionalDemoVariant, FutureDemoConfig> = {
  ...
  fsaz: {
    courses: [
      { id: "fsaz-grund", title: "Grundausbildung · 6 × 45 Min.", location: "FSAZ · Hamburg-Rahlstedt", start: "Start nach persönlicher Abstimmung", time: "Bedienung, Anfahren, Schalten und sichere Routinen", seats: 1, tag: "Empfohlener Einstieg", match: "Passt zu: wenig Erfahrung, Nervosität oder ruhigem Einstieg" },
      { id: "fsaz-komplett", title: "Komplettpaket · 9 × 45 Min.", location: "FSAZ · Hamburg-Rahlstedt", start: "Nach Trainingsziel abstimmen", time: "Grundausbildung plus weiterführende Module", seats: 1, tag: "Kompletter Trainingsweg", match: "Passt zu: strukturiertem Simulatortraining vor und neben der Praxis" },
      { id: "fsaz-module", title: "Überland / Autobahn / Automatik", location: "FSAZ · Hamburg-Rahlstedt", start: "Einzelmodul nach Bedarf", time: "Gezielte Situationen ohne reales Verkehrsrisiko vorbereiten", seats: 1, tag: "Gezielt trainieren", match: "Passt zu: konkreten Unsicherheiten oder Auffrischung" },
    ],
    documents: [
      ...
      { id: "kontakt", label: "Kontakt für Rückmeldung", detail: "Mit FSAZ abstimmen", initial: true },
    ],
    ...
    connectPotential: "Du kannst bei deiner bisherigen Fahrschule bleiben. FSAZ kann als eigenständiges Simulatortraining zusätzlich zu deiner laufenden Fahrausbildung genutzt werden.",
    ...
    scheduleDescription: "Hier werden Wunschzeiten als Demo gebündelt. Echte freie Simulator-Slots werden nicht behauptet, sondern mit dem FSAZ-Team abgestimmt.",
...
const FSAZ_TAB_ITEMS: ReadonlyArray<{ id: HohenbadenPanel; label: string }> = [
  { id: "home", label: "Start" },
  { id: "dashboard", label: "So funktioniert der Simulator" },
  { id: "courses", label: "Training & Module" },
  { id: "coach", label: "Preise" },
  { id: "documents", label: "Häufige Fragen" },
];
...
function FsazTabBar(props: RathjeTabBarProps) {
  const { activePanel, onPanelChange, accent, accentRgb, textPrimary, textSecondary } = props;
  const selected = activePanel === "connect" || activePanel === "schedule" ? "courses" : activePanel;
  return (
    <nav className="bt-fsaz-tabs" aria-label="FSAZ-Bereiche" style={{ width: "100%", display: "flex", gap: 7, overflowX: "auto", padding: 4, boxSizing: "border-box", borderRadius: 18, border: "1px solid rgba(255,255,255,0.58)", background: "rgba(255,255,255,0.48)", boxShadow: "0 10px 30px rgba(30,45,60,0.07), inset 0 1px 0 rgba(255,255,255,0.78)", backdropFilter: "blur(22px) saturate(165%)", WebkitBackdropFilter: "blur(22px) saturate(165%)" }}>
      {FSAZ_TAB_ITEMS.map((item) => {
        const active = selected === item.id;
        return <button key={item.id} type="button" aria-current={active ? "page" : undefined} onClick={() => onPanelChange(item.id)} style={{ flex: "1 0 auto", minHeight: 38, borderRadius: 14, border: active ? `1px solid rgba(${accentRgb}, 0.34)` : "1px solid transparent", background: active ? `linear-gradient(180deg, rgba(${accentRgb}, 0.20), rgba(${accentRgb}, 0.11))` : "transparent", color: active ? textPrimary : textSecondary, padding: "0 12px", fontSize: 12, fontWeight: active ? 950 : 820, cursor: "pointer", whiteSpace: "nowrap", boxShadow: active ? `0 8px 20px rgba(${accentRgb}, 0.12)` : "none" }}>
          {active && <span aria-hidden="true" style={{ display: "inline-block", width: 6, height: 6, borderRadius: 999, marginRight: 7, verticalAlign: 1, background: accent }} />}{item.label}
        </button>;
      })}
    </nav>
  );
...
function RathjeInteractivePanel({
      ...
      ? [
          {
            id: "license",
            eyebrow: "SCHRITT 1",
            question: "Für welche Führerscheinklasse möchtest du die Kosten sehen?",
            helper: "Danach zeigen wir nur noch die dafür relevanten Positionen.",
            choices: [
              { label: "Klasse B", value: "Klasse B" },
              { label: "B197", value: "B197" },
              { label: "B78 Automatik", value: "B78" },
          ...
          },
          {
            id: "priceNeed",
            eyebrow: "SCHRITT 2",
            question: "Welche Kosteninformation brauchst du?",
            choices: [
              { label: "Startkosten", value: "Startkosten", detail: "Grundbetrag, Lernmittel und Büroservice" },
              { label: "Fahrstunden", value: "Fahrstundenpreise", detail: "Übungs- und Sonderfahrten einordnen" },
              { label: "Gesamtüberblick", value: "Gesamtüberblick", detail: "Alle veröffentlichten Positionen" },
            ],
          },
        ]
      : panel === "schedule"
        ? [
            {
              ...
              choices: [
                { label: "Theorie", value: "Theorie", detail: "Unterricht und Einstieg" },
                { label: "Fahrstunden", value: "Fahrstunden", detail: "Praxis passend zum Alltag" },
                { label: "Simulator", value: "Simulator", detail: "Gezielte Vorbereitung" },
              ],
            },
            {
              id: "phase",
              ...
              choices: [
                { label: "Noch nicht gestartet", value: "Noch nicht gestartet" },
                { label: "Theorie läuft", value: "Theorie läuft" },
                { label: "Praxis läuft", value: "Praxis läuft" },
                { label: "Prüfung vorbereiten", value: "Prüfungsvorbereitung" },
              ],
            },
            {
              id: "availability",
              eyebrow: "SCHRITT 3",
              question: "Wann passt es dir grundsätzlich am besten?",
              choices: [
            ...
            },
            {
              id: "planningGoal",
              eyebrow: "SCHRITT 4",
              question: "Was soll Rathje für dich als Nächstes klären?",
              choices: [
                { label: "Passenden Einstieg", value: "Passenden Einstieg vorschlagen" },
                { label: "Nächsten freien Schritt", value: "Nächsten möglichen Schritt prüfen" },
                { label: "Persönlichen Plan", value: "Persönlichen Ausbildungsplan erstellen" },
              ],
            },
          ]
        : [
            {
              id: "origin",
              eyebrow: "SCHRITT 1",
              question: "Was ist deine aktuelle Ausgangssituation?",
              choices: [
                { label: "Wechsel von einer anderen Fahrschule", value: "andere Fahrschule" },
                { label: "Umzug oder Standortwechsel", value: "Umzug / Standortwechsel" },
                { label: "Ausbildung länger unterbrochen", value: "unterbrochene Ausbildung" },
                { label: "Erst prüfen, ob ein Wechsel passt", value: "unverbindliche Wechselprüfung" },
              ],
            },
            {
              id: "license",
              eyebrow: "SCHRITT 2",
              question: "Welche Ausbildung möchtest du zu Rathje übernehmen?",
              choices: [
                { label: "B / B197 / B78", value: "B / B197 / B78" },
                { label: "BF17", value: "BF17" },
                { label: "BE / Anhänger", value: "BE / Anhänger" },
                { label: "Andere Klasse", value: "Andere Klasse" },
              ],
            },
            {
              id: "status",
              eyebrow: "SCHRITT 3",
              question: "Wie weit bist du bei deiner bisherigen Fahrschule?",
              choices: [
                { label: "Gerade angemeldet", value: "Anmeldung erfolgt" },
                { label: "Theorie läuft", value: "Theorie läuft" },
            ...
            },
            {
              id: "theoryExam",
              eyebrow: "SCHRITT 4",
              question: "Welchen Status hat deine Theorieprüfung?",
              choices: [
                { label: "Noch nicht angemeldet", value: "noch nicht angemeldet" },
                { label: "Angemeldet", value: "angemeldet" },
                { label: "Bestanden", value: "bestanden" },
                { label: "Nicht bestanden / neuer Versuch", value: "neuer Versuch nötig" },
              ],
            },
            {
              id: "completed",
              eyebrow: "SCHRITT 5",
              question: "Welche Ausbildungsschritte hast du bereits absolviert?",
              choices: [
                ...
                { label: "Sonderfahrten", value: "Übungs- und Sonderfahrten" },
              ],
            },
            {
              id: "documents",
              eyebrow: "SCHRITT 6",
              question: "Welche Unterlagen kannst du bereits vorlegen?",
              choices: [
                { label: "Ausbildungsnachweis vollständig", value: "vollständiger Ausbildungsnachweis" },
                { label: "Einige Nachweise vorhanden", value: "einige Nachweise vorhanden" },
                { label: "Nachweise fehlen", value: "Nachweise fehlen" },
  ...
  const title = panel === "coach" ? "Persönliche Preisübersicht" : panel === "schedule" ? "Theorie & Fahrstunden planen" : "Fahrschulwechsel vorbereiten";
  const successTitle = panel === "documents" ? "Wechselanfrage wurde erfolgreich übermittelt" : "Dein persönliches Ergebnis ist fertig";

  const labels: Record<string, string> = {
    license: "Führerscheinklasse",
    priceNeed: "Gewünschte Übersicht",
    situation: "Verwendung",
    area: "Bereich",
    phase: "Ausbildungsstand",
    availability: "Verfügbarkeit",
    planningGoal: "Planungsziel",
    status: "Bisheriger Stand",
    origin: "Ausgangssituation",
  ...
  function getRathjePriceResult() {
    const license = answers.license || "Klasse B";
    const isBe = license === "BE";
    const startTotal = isBe ? "285 €" : "584,90 €";
    const lessonPrice = isBe ? "75 €" : "70 €";
    const lines = isBe
  ...
  function getRathjeScheduleResult() {
    ...
    const phaseName = answers.phase || "Noch nicht gestartet";
    const next = area === "Theorie"
      ? phaseName === "Prüfungsvorbereitung" ? "Besuche die noch offenen Themen und gleiche danach deinen Lernstand mit Rathje ab." : "Steige in den nächsten passenden Theorieunterricht ein und arbeite die Themen parallel nach."
      : area === "Fahrstunden"
        ? phaseName === "Noch nicht gestartet" ? "Klasse und Unterlagen zuerst bestätigen, danach die erste Praxisetappe mit Rathje abstimmen." : "Den bisherigen Lernstand mit dem Fahrlehrer abgleichen und darauf die nächste Praxisetappe aufbauen."
        : "Simulatorziel festlegen, die Einheit als Vorbereitung nutzen und die Inhalte anschließend in der Straßenpraxis übertragen.";
    const goalAddition = answers.planningGoal === "Persönlichen Ausbildungsplan erstellen"
      ? " Ordne danach Theorie, Praxis und gegebenenfalls Simulator als aufeinanderfolgende Etappen."
      : answers.planningGoal === "Nächsten möglichen Schritt prüfen"
        ? " Lass als Nächstes genau diesen Schritt persönlich auf Verfügbarkeit prüfen."
        : " Beginne mit diesem Einstieg und prüfe danach den Lernstand erneut.";
    return { area, next: `${next}${goalAddition}`, preferred: answers.availability || "Flexibel", showTheoryTimes: area === "Theorie" };
  }

  function getRathjeTransferResult() {
    const documentState = answers.documents || "Unterlagenprüfung nötig";
    const completed = answers.completed || "bisherige Ausbildungsschritte";
    const exam = answers.theoryExam || "ungeklärter Theorieprüfungsstatus";
    const available = documentState === "vollständiger Ausbildungsnachweis" ? `Ausbildungsnachweis und bisheriger Ausbildungsstand sind als vorhanden markiert. Erfasst sind ${completed}; Theorieprüfung: ${exam}.` : documentState === "einige Nachweise vorhanden" ? `Ein Teil deiner Ausbildungsnachweise ist vorhanden. Angegeben sind ${completed}; Theorieprüfung: ${exam}.` : `Es sind noch keine belastbaren Ausbildungsnachweise markiert. Angegeben wurden ${completed}; Theorieprüfung: ${exam}.`;
    const missing = documentState === "vollständiger Ausbildungsnachweis" ? "Rathje prüft noch Fahrerlaubnisantrag beziehungsweise Prüfauftrag und die Gültigkeit der Nachweise." : "Vermutlich fehlen Teile des Ausbildungsnachweises, Angaben zu Theorie- und Praxisstunden oder der Prüfstatus.";
    const request = documentState === "vollständiger Ausbildungsnachweis" ? "Bitte die bisherige Fahrschule nur um noch fehlende Einzelbelege und eine Bestätigung des Ausbildungsstands." : "Fordere bei der bisherigen Fahrschule den vollständigen Ausbildungsnachweis mit Theorieunterricht, Übungsfahrten, Sonderfahrten und Prüfungsstatus an.";
    return { available, missing, request, next: "Rathje kann die Nachweise anschließend prüfen, anrechenbare Leistungen einordnen und den verbleibenden Ausbildungsweg persönlich festlegen." };
  }
    ...
    <section className="bt-rathje-interactive bt-guided-flow" style={{ ...glass, padding: isMobile ? 14 : 19, display: "flex", flexDirection: "column", gap: 14, color: textPrimary, flex: "0 0 auto" }}>
        ...
        <div className="bt-guided-step-in" style={{ display: "flex", flexDirection: "column", gap: 11 }}>
          ...
          {panel === "documents" && <>
            ...
            <label style={{ ...soft, padding: 14, cursor: "pointer" }}>
              ...
              {files.length > 0 && <span style={{ display: "block", color: accent, fontSize: 11.5, fontWeight: 900, marginTop: 7 }}>{files.length} Datei{files.length === 1 ? "" : "en"} zur Demo-Zusammenfassung hinzugefügt</span>}
            </label>
            <form onSubmit={submitDemo} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 9 }}><input value={contactName} onChange={(event) => setContactName(event.target.value)} placeholder="Vor- und Nachname" style={field} /><input value={contactValue} onChange={(event) => setContactValue(event.target.value)} placeholder="E-Mail oder Telefonnummer" style={field} /></div>
              <textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional: kurze Ergänzung für Rathje" rows={2} style={{ ...field, minHeight: 68, padding: 12, resize: "vertical" }} />
              <label style={{ ...soft, padding: 12, display: "flex", gap: 9, alignItems: "flex-start", color: textSecondary, fontSize: 11.5, lineHeight: 1.4 }}><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} />Ich bestätige den Demo-Hinweis. Es werden keine Daten oder Dateien wirklich versendet.</label>
              {error && <div style={{ color: "#a23b3b", fontSize: 12.5, fontWeight: 850 }}>{error}</div>}
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "0.8fr 1.2fr", gap: 9 }}><button type="button" onClick={goBack} style={secondary}>Angaben ändern</button><button type="submit" style={primary}>Demo-Wechselanfrage absenden</button></div>
            </form>
          </>}
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, minmax(0, 1fr))", gap: 9 }}>
            ...
            <button type="button" onClick={restart} style={secondary}>{panel === "coach" ? "Andere Führerscheinklasse prüfen" : "Neuen Plan erstellen"}</button>
      ...
      {phase === "success" && (
        <div className="bt-guided-step-in" style={{ ...soft, padding: isMobile ? 17 : 21 }}>
          ...
          <div style={{ color: accent, fontSize: 11, fontWeight: 950, letterSpacing: 0.46, marginTop: 14 }}>DEMO · ERFOLGREICH BEI RATHJE EINGEGANGEN</div>
          <div style={{ fontSize: isMobile ? 21 : 25, fontWeight: 950, marginTop: 5 }}>{successTitle}</div>
          <div style={{ color: textSecondary, fontSize: 12.5, lineHeight: 1.52, marginTop: 7 }}>Rathje würde jetzt die geordneten Angaben von {contactName} inklusive Kontakt, Auswahl{files.length ? ` und ${files.length} Datei${files.length === 1 ? "" : "en"}` : ""} erhalten. In dieser Demo wurde nichts wirklich versendet.</div>
...
function GuidedTriDemo({
    ...
    if (isRathje) {
      if (panel === "courses") {
        return [
          ...
          ...(answers.license === "BE" || answers.license === "BF17" || answers.license === "unsicher" ? [{
            id: "startingPoint",
            eyebrow: "PASSENDE FOLGEFRAGE",
            question: answers.license === "BE" ? "Hast du die Klasse B bereits?" : answers.license === "BF17" ? "Wie alt bist du beim gewünschten Start?" : "Wofür brauchst du den Führerschein hauptsächlich?",
            choices: answers.license === "BE" ? [
              { label: "B bereits vorhanden", value: "Klasse B vorhanden" },
              { label: "B-Ausbildung läuft", value: "Klasse B in Ausbildung" },
              { label: "Noch nicht", value: "Klasse B noch nicht vorhanden" },
            ] : answers.license === "BF17" ? [
              { label: "Noch unter 16½", value: "unter 16½ Jahre" },
              { label: "16½ bis 17", value: "16½ bis 17 Jahre" },
              { label: "Schon 17 oder älter", value: "17 Jahre oder älter" },
            ] : [
              { label: "Auto im Alltag", value: "Auto im Alltag" },
              { label: "Begleitet ab 17", value: "Begleitet ab 17" },
              { label: "Automatik und Schaltung", value: "Automatik und Schaltung" },
              { label: "Anhänger fahren", value: "Anhänger fahren" },
            ],
          } satisfies GuidedQuestion] : []),
          {
            id: "documents",
            eyebrow: "STARTUNTERLAGEN",
            question: "Wie weit bist du mit deinen Startunterlagen?",
            choices: [
              { label: "Noch nichts vorbereitet", value: "nichts vorbereitet" },
              { label: "Teilweise vorhanden", value: "teilweise vorhanden" },
              { label: "Alles vorbereitet", value: "vollständig" },
              { label: "Ich brauche eine Checkliste", value: "Checkliste benötigt" },
            ],
          },
          {
            id: "time",
            eyebrow: "AUSBILDUNGSSTART",
            question: "Wann möchtest du ungefähr starten?",
            choices: [
              { label: "So bald wie möglich", value: "so bald wie möglich" },
              { label: "In 1–2 Monaten", value: "in 1–2 Monaten" },
              { label: "Später im Jahr", value: "später im Jahr" },
              { label: "Noch offen", value: "Startzeit noch offen" },
            ],
          },
        ];
      }
  ...
  const requiresDemoSubmission = isRathje && panel === "courses";
  ...
  function resultText() {
    ...
    if (isRathje) {
      if (panel === "courses") {
        const inferredClass = answers.license === "unsicher"
          ? answers.startingPoint === "Begleitet ab 17" ? "BF17" : answers.startingPoint === "Anhänger fahren" ? "BE" : answers.startingPoint === "Automatik und Schaltung" ? "B197" : "Klasse B"
          : answers.license || "Führerschein";
        const prerequisite = inferredClass === "BF17"
          ? answers.startingPoint === "unter 16½ Jahre" ? "Du kannst die Vorbereitung bereits planen; der reguläre BF17-Start ist erst ab 16½ Jahren möglich." : "Für BF17 werden Alter, Zustimmung der Erziehungsberechtigten und geeignete Begleitpersonen persönlich geprüft."
          : inferredClass === "BE" ? (answers.startingPoint === "Klasse B vorhanden" ? "Die Voraussetzung Klasse B ist nach deiner Angabe erfüllt." : "Vor dem BE-Start muss Rathje den Stand deiner Klasse B prüfen.")
          : "Rathje prüft zum Start die gewählte Ausbildungsvariante und den Fahrerlaubnisantrag.";
        const missing = answers.documents === "vollständig" ? "Deine Startunterlagen sind als vollständig markiert; Rathje prüft sie vor Ort auf Gültigkeit." : "Voraussichtlich zu prüfen oder nachzureichen sind Ausweis, biometrisches Passbild, Sehtest, Erste-Hilfe-Nachweis und Fahrerlaubnisantrag.";
        return `Empfohlener Start: ${inferredClass}. ${prerequisite} ${missing} Dein Ablauf: 1. Unterlagen prüfen, 2. Anmeldung und Theorieeinstieg persönlich abstimmen, 3. Praxis beziehungsweise Simulator passend zum Lernstand planen. Als Wunschstart ist „${answers.time || "offen"}“ hinterlegt.`;
      }
      if (panel === "connect") {
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: isMobile ? 24 : 29, fontWeight: 950, marginTop: 4 }}>
          {isComplete
            ? requiresDemoSubmission && !demoSubmitted
              ? "Dein persönlicher Rathje-Startplan"
      ...
      {isComplete && (
        <>
          ...
          >
            ...
            <div>
              <div style={{ ...smallLabel, color: accent }}>
                {requiresDemoSubmission && !demoSubmitted
              ...
              {requiresDemoSubmission && demoSubmitted && (
                <div style={{ color: textSecondary, fontSize: 12.5, lineHeight: 1.5, marginTop: 7 }}>
                  Die Demo-Anfrage wurde erfolgreich verarbeitet. Es wurden keine echten Daten übertragen.
                </div>
              )}
            </div>
          </div>}
          ...
          {requiresDemoSubmission && !demoSubmitted && (
            <div style={{ ...glass, padding: 14 }}>
              <div style={{ ...smallLabel }}>OPTIONALE DEMO-STARTANFRAGE</div>
              <div style={{ color: textSecondary, fontSize: 12.5, lineHeight: 1.5, marginTop: 6 }}>Erst nach dem fertigen Startplan kannst du Kontaktdaten für die Demo ergänzen. Es wird nichts wirklich versendet oder gespeichert.</div>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 9, marginTop: 11 }}>
                <input value={demoName} onChange={(event) => { setDemoName(event.target.value); setDemoError(""); }} placeholder="Vor- und Nachname" style={{ minHeight: 45, borderRadius: 13, border: `1px solid rgba(${accentRgb}, 0.17)`, background: "rgba(255,255,255,0.82)", color: textPrimary, padding: "0 12px", outline: "none", fontSize: 13.5, minWidth: 0 }} />
                <input value={demoContact} onChange={(event) => { setDemoContact(event.target.value); setDemoError(""); }} placeholder="E-Mail oder Telefonnummer" style={{ minHeight: 45, borderRadius: 13, border: `1px solid rgba(${accentRgb}, 0.17)`, background: "rgba(255,255,255,0.82)", color: textPrimary, padding: "0 12px", outline: "none", fontSize: 13.5, minWidth: 0 }} />
              </div>
              {demoError && <div style={{ color: "#a23b3b", fontSize: 12.5, fontWeight: 850, marginTop: 8 }}>{demoError}</div>}
            </div>
          )}
          ...
          >
            {requiresDemoSubmission && demoSubmitted ? (
              <button type="button" onClick={() => onPanelChange("home")} style={{ ...secondary, minHeight: 49 }}>
                Zur Übersicht
              </button>
            ) : (
            ...
            {requiresDemoSubmission ? (
              demoSubmitted ? (
                <button type="button" onClick={restartFlow} style={{ ...primary, minHeight: 49 }}>
                  Neue Demo-Anfrage
                </button>
              ) : (
                <button type="button" onClick={() => { if (!demoName.trim() || !demoContact.trim()) { setDemoError("Bitte ergänze deinen Namen und eine Kontaktmöglichkeit."); return; } setDemoError(""); setDemoSubmitted(true); }} style={{ ...primary, minHeight: 49 }}>
                  Demo-Anfrage absenden
                </button>
              )
            ) : (
              isRathje ? (
                <button type="button" onClick={() => onPanelChange("home")} style={{ ...primary, minHeight: 49 }}>Zur Übersicht</button>
              ) : (
                <button type="button" onClick={finishInChat} style={{ ...primary, minHeight: 49 }}>Im Chat weiterführen</button>
              )
            )}
          </div>
        </>
...
type FsazAdvicePhase = "questions" | "review" | "success";

function FsazInteractiveDemo({
  panel,
  onPanelChange,
  accent,
  accentRgb,
  ...
  const activePanel: HohenbadenPanel =
    panel === "connect" || panel === "schedule" ? "documents" : panel;
  const [step, setStep] = useState(0);
  const [phase, setPhase] = useState<FsazAdvicePhase>("questions");
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [freeText, setFreeText] = useState("");
  ...
  const glass: CSSProperties = {
    ...
    boxShadow: "0 18px 54px rgba(25,55,76,0.10), inset 0 1px 0 rgba(255,255,255,0.84)",
  ...
  const primary: CSSProperties = {
    ...
    borderRadius: 14,
    background: `linear-gradient(180deg, ${accent}, ${accent}D5)`,
    color: "#fff",
    padding: "0 16px",
    fontWeight: 900,
    cursor: "pointer",
    boxShadow: `0 11px 27px rgba(${accentRgb}, 0.20)`,
  };
  const secondary: CSSProperties = {
    minHeight: 43,
    border: `1px solid rgba(${accentRgb}, 0.17)`,
      ...
      return [
        {
          id: "situation",
          eyebrow: "SCHRITT 1",
          question: "Welche Situation passt zu dir?",
          choices: [
            { label: "Ich starte meine Ausbildung", value: "Ausbildungsstart" },
            { label: "Ich fahre bereits in der Ausbildung", value: "laufende Ausbildung" },
            { label: "Ich möchte nach einer Pause auffrischen", value: "Auffrischung" },
            { label: "Ich möchte den Simulator kennenlernen", value: "Information" },
          ],
        },
        ...(answers.situation === "Ausbildungsstart" || answers.situation === "laufende Ausbildung" || answers.situation === "Auffrischung" ? [{
          id: "experience",
          eyebrow: "FAHRERFAHRUNG",
          question: "Wie viel Fahrerfahrung bringst du bereits mit?",
          choices: [
            { label: "Noch keine", value: "keine" },
            { label: "Einige Fahrstunden", value: "erste Fahrstunden" },
            { label: "Schon länger gefahren", value: "erfahren" },
          ],
        } satisfies GuidedQuestion] : []),
        {
          id: "interest",
          eyebrow: "DEIN INTERESSE",
          question: "Was möchtest du am Simulator verstehen?",
          choices: [
            { label: "Ablauf vor Ort", value: "Ablauf" },
            { label: "Technik & Bedienung", value: "Technik" },
            { label: "Nutzen fürs Fahren", value: "Nutzen" },
            { label: "Grenzen des Simulators", value: "Grenzen" },
          ],
        },
        {
          id: "goal",
          eyebrow: "TRAININGSZIEL",
          question: "Welchen Bereich möchtest du genauer kennenlernen?",
          choices: [
            { label: "Grundlagen", value: "Grundlagen" },
            { label: "Schalten & Anfahren", value: "Schalten" },
            { label: "Überland", value: "Überland" },
            { label: "Autobahn", value: "Autobahn" },
            { label: "Allgemeiner Ablauf", value: "Ablauf" },
          ],
        },
        {
          id: "detail",
          eyebrow: "ERKLÄRUNGSTIEFE",
          question: "Wie ausführlich soll die Erklärung sein?",
          choices: [
            { label: "Kurz und konkret", value: "kurz" },
            { label: "Mit Beispielablauf", value: "ausführlich" },
            { label: "Mit Nutzen und Grenzen", value: "vertieft" },
          ],
        },
      ];
    }
      ...
      return [
        {
          id: "goal",
          eyebrow: "SCHRITT 1",
          question: "Was möchtest du gezielt üben?",
          choices: [
            { label: "Ruhig einsteigen", value: "Grundlagen und Bedienung" },
            { label: "Schalten & Anfahren", value: "Schalten und Anfahren" },
            { label: "Überland", value: "Überland" },
            { label: "Autobahn", value: "Autobahn" },
            { label: "Automatik", value: "Automatik" },
            { label: "Noch unsicher", value: "Empfehlung benötigt" },
          ],
        },
        {
          id: "experience",
          ...
          choices: [
            { label: "Noch keine", value: "keine Erfahrung" },
            { label: "Ein paar Fahrstunden", value: "erste Erfahrung" },
            { label: "Schon recht sicher", value: "fortgeschritten" },
            { label: "Ich möchte auffrischen", value: "Auffrischung" },
          ],
        },
        {
          id: "school",
          eyebrow: "SCHRITT 3",
          question: "Wo machst du deine Fahrausbildung?",
          helper: "Das ändert nur die Preis- und Ablaufhinweise dieser Übersicht.",
          choices: [
            { label: "Bei Rathje", value: "Fahrschule Rathje" },
            { label: "Andere Fahrschule", value: "externe Fahrschule" },
            { label: "Noch nicht angemeldet", value: "noch keine Fahrschule" },
            { label: "Nur Information", value: "unverbindliche Information" },
          ],
        },
      ];
    }
      ...
      return [
        {
          ...
          question: "Wo machst du deine Fahrausbildung?",
          choices: [
            { label: "Rathje-Fahrschüler", value: "Fahrschule Rathje" },
            { label: "Andere Fahrschule", value: "externe Fahrschule" },
            { label: "Noch nicht angemeldet", value: "noch keine Fahrschule" },
            { label: "Nur vergleichen", value: "unverbindlicher Vergleich" },
          ],
        },
        {
          id: "goal",
          eyebrow: "SCHRITT 2",
          question: "Was möchtest du hauptsächlich trainieren?",
          choices: [
            { label: "Grundlagen", value: "Grundlagen" },
            { label: "Schalten & Anfahren", value: "Schalten und Anfahren" },
            { label: "Mehrere Bereiche", value: "mehrere Bereiche" },
            { label: "Auffrischung", value: "Auffrischung" },
          ],
        },
        {
          id: "experience",
          eyebrow: "SCHRITT 3",
          question: "Wie viel Fahrerfahrung hast du?",
          choices: [
            { label: "Noch keine", value: "keine" },
            { label: "Erste Fahrstunden", value: "erste Fahrstunden" },
            { label: "Schon recht sicher", value: "erfahren" },
            { label: "Länger nicht gefahren", value: "Auffrischung" },
          ],
        },
        {
          id: "package",
          eyebrow: "SCHRITT 4",
          question: "Welchen Umfang möchtest du prüfen?",
          choices: [
            { label: "6 × 45 Minuten", value: "Grundausbildung" },
            { label: "9 × 45 Minuten", value: "Komplettpaket" },
            { label: "Bitte empfehlen", value: "Paketempfehlung" },
          ],
        },
      ];
    }
    ...
    return [
      {
        id: "topic",
        eyebrow: "SCHRITT 1",
        question: "Welche Frage möchtest du klären?",
        choices: [
          { label: "Ersetzt das Fahrstunden?", value: "Ersatz von Fahrstunden" },
          { label: "Training als externer Schüler", value: "Training für Externe" },
          { label: "Wie läuft die Anmeldung?", value: "Anmeldung" },
          { label: "Wie komme ich an einen Termin?", value: "Termin" },
          { label: "Welches Paket passt?", value: "Paketwahl" },
        ],
      },
      {
        id: "situation",
        eyebrow: "SCHRITT 2",
        question: "Welche Situation soll die Antwort berücksichtigen?",
        choices: [
          { label: "Noch nicht angemeldet", value: "noch nicht angemeldet" },
          { label: "Bei Rathje", value: "bei Rathje" },
          { label: "Andere Fahrschule", value: "bei einer anderen Fahrschule" },
          { label: "Nur informieren", value: "allgemeine Information" },
        ],
      },
    ];
  }
  ...
  function choose(value: string) {
    ...
    questions.slice(step + 1).forEach((question) => delete next[question.id]);
    setAnswers(next);
    setFreeText("");
    if (step + 1 >= questions.length) setPhase("success");
    else setStep((current) => current + 1);
  }
  ...
  function recommendation() {
    if (activePanel === "dashboard") {
      ...
      const suggested = goal === "Ablauf" ? "eine Einführung mit anschließendem Grundlagenmodul" : beginner && (goal === "Überland" || goal === "Autobahn") ? `zuerst die Grundausbildung und danach das Modul ${goal}` : goal === "Schalten" ? "das Modul Schalten und Anfahren" : goal === "Grundlagen" ? "die Grundausbildung" : `das Modul ${goal}`;
      const practice = goal === "Schalten" ? "Pedalgefühl, Kupplung, Anfahren und wiederholbare Schaltabläufe" : goal === "Überland" ? "Tempoanpassung, Blickführung und Entscheidungen auf Landstraßen" : goal === "Autobahn" ? "Auffahren, Spurwechsel, Abstand und vorausschauendes Fahren" : "Fahrzeugbedienung, Blickführung und sichere Grundabläufe";
      const benefit = answers.situation === "Auffrischung" ? "Du kannst vertraute Abläufe nach einer Pause ohne Verkehrsdruck wiederholen." : beginner ? "Du lernst die ersten Abläufe in Ruhe kennen und gehst vorbereitet in die echte Fahrstunde." : "Du kannst einzelne Unsicherheiten gezielt und mehrfach durchspielen.";
      const focus = answers.interest === "Technik" ? "Du arbeitest an einem fahrzeugnahen Cockpit mit Pedalen, Schaltung und Sichtfeld." : answers.interest === "Grenzen" ? "Die Rückmeldung im Simulator ist eine Vorbereitung; Verkehrsgefühl und echte Interaktion entstehen erst auf der Straße." : "Die Einheit beginnt mit einer Einweisung, dann folgt eine passende Übung mit Besprechung und Wiederholung.";
      return `Für dich könnte ${suggested} sinnvoll sein. ${focus} Dabei trainierst du ${practice}. ${benefit} ${answers.detail === "kurz" ? "" : "Das Training wird deinem Erfahrungsstand angepasst; schwierige Situationen können kontrolliert wiederholt und anschließend besprochen werden. "}Der Simulator ergänzt die praktische Fahrausbildung auf der Straße und ersetzt sie nicht. Reale Verkehrsteilnehmer, Wetter, Fahrgefühl und die gesetzlich erforderliche Praxis lassen sich damit nicht vollständig abbilden.`;
    }
    if (activePanel === "courses") {
      const external = answers.school === "externe Fahrschule"
        ? " Für externe Fahrschüler kommt laut veröffentlichter Preisliste eine einmalige Anmeldung von 30 € hinzu."
        : "";
      const goal = answers.goal || "Grundlagen und Bedienung";
      const content = goal.includes("Schalten") ? "Kupplungspunkt, Gasdosierung, Gangwechsel und kontrolliertes Anfahren" : goal === "Überland" ? "Blickführung, Tempoanpassung, Kurven und vorausschauende Entscheidungen" : goal === "Autobahn" ? "Auffahren, Abstand, Spurwechsel und Geschwindigkeit" : goal === "Automatik" ? "Fahrzeugbedienung, Bremsdosierung und sichere Routinen ohne Schaltvorgang" : "Sitzposition, Pedale, Lenken, Blickführung und sichere Grundabläufe";
      const packageName = answers.experience === "keine Erfahrung" || goal.includes("Grundlagen") ? "Grundausbildung mit 6 × 45 Minuten" : "gezieltes Einzelmodul; bei mehreren Trainingszielen das Komplettpaket mit 9 × 45 Minuten";
      return `Empfohlenes Modul: ${goal}. Dieses Modul passt zu deinem Erfahrungsstand „${answers.experience || "offen"}“, weil die Schwierigkeit kontrolliert aufgebaut und einzelne Abläufe wiederholt werden können. Trainiert werden ${content}. Passende Preisoption: ${packageName}. Die genaue Moduleinteilung und Teilnahme werden persönlich vor Ort geklärt.${external}`;
    }
    if (activePanel === "coach") {
      const recommendedPackage = answers.package === "Paketempfehlung"
      ...
      const base = recommendedPackage === "Komplettpaket" ? 270 : 180;
      const externalFee = answers.school === "externe Fahrschule" ? 30 : 0;
      const units = recommendedPackage === "Komplettpaket" ? "9 × 45 Minuten" : "6 × 45 Minuten";
      const reason = recommendedPackage === "Komplettpaket" ? "Mehrere Trainingsbereiche beziehungsweise eine breitere Auffrischung brauchen mehr Raum für Aufbau und Wiederholung." : "Für einen klar begrenzten Einstieg oder Grundlagenziel ist der kompakte Umfang passend.";
      return `Empfehlung: ${recommendedPackage} mit ${units}. Preis: ${base} €. ${answers.package === "Paketempfehlung" ? reason : "Du hast diesen Umfang selbst ausgewählt."}${externalFee ? ` Einmalige Anmeldung für externe Fahrschüler: 30 €. Gesamt: ${base + externalFee} €.` : ` Gesamt: ${base} €.`} Den genauen Umfang und die aktuell gültigen Konditionen bestätigt FSAZ persönlich vor Ort.`;
    }
    if (answers.topic === "Ersatz von Fahrstunden") return "Der Simulator ergänzt die Fahrausbildung, ersetzt aber keine vorgeschriebenen oder praktisch notwendigen Fahrstunden auf der Straße.";
    if (answers.topic === "Training für Externe") return "Zusätzliches Simulatortraining kann auch bei einer anderen Fahrschule infrage kommen. Ein Fahrschulwechsel ist dafür nicht automatisch nötig; Details werden vor Ort geklärt.";
    if (answers.topic === "Anmeldung") return "Die Anmeldung ist ausschließlich persönlich beim FSAZ-Team vor Ort möglich. Dieses Interface nimmt keine Anmeldung entgegen.";
    if (answers.topic === "Termin") return "Termine und Verfügbarkeiten werden ausschließlich vor Ort geklärt. Dieses Interface bietet weder Buchung noch Reservierung an.";
  ...
  const flowNames: Record<string, string> = {
    ...
    coach: "Preise einordnen",
    documents: "Frage klären",
  };

  return (
    <section
      className="bt-fsaz-info bt-guided-flow"
      style={{ ...glass, alignSelf: "stretch", padding: isMobile ? 14 : 19, display: "flex", flexDirection: "column", gap: 14, color: textPrimary, flex: "0 0 auto" }}
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <button type="button" onClick={goBack} style={secondary}>
          {phase !== "questions" || step > 0 ? "← Zurück" : "← Übersicht"}
        </button>
        <div style={{ borderRadius: 999, padding: "8px 11px", background: `rgba(${accentRgb}, 0.09)`, border: `1px solid rgba(${accentRgb}, 0.15)`, color: textSecondary, fontSize: 11.5, fontWeight: 900 }}>
          FSAZ · interaktive Information
        </div>
      </div>
      ...
      {phase !== "success" && <div>
        ...
        <div style={{ fontSize: isMobile ? 24 : 30, fontWeight: 950, lineHeight: 1.14, marginTop: 5 }}>
          {phase === "questions"
            ? currentQuestion?.question
            : phase === "review"
              ? "Deine persönliche Übersicht ist vorbereitet."
              : "Deine persönliche FSAZ-Übersicht wurde erfolgreich erstellt."}
        </div>
        <div style={{ color: textSecondary, fontSize: 13, lineHeight: 1.5, marginTop: 7 }}>
          {phase === "questions"
            ? currentQuestion?.helper || "Wähle eine Antwort. Danach erscheint automatisch nur der nächste passende Schritt."
            : phase === "review"
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
        <div style={{ flex: 1, height: 7, borderRadius: 999, background: `rgba(${accentRgb}, 0.10)`, overflow: "hidden" }}>
          <div style={{ width: `${phase === "questions" ? Math.max(10, ((step + 1) / questions.length) * 100) : 100}%`, height: "100%", borderRadius: 999, background: accent, transition: "width 220ms ease" }} />
        </div>
        <span style={{ color: textSecondary, fontSize: 11.5, fontWeight: 850 }}>
          {phase === "questions" ? `${step + 1}/${questions.length}` : phase === "review" ? "Prüfen" : "Fertig"}
        </span>
      </div>
      ...
      {phase === "questions" && currentQuestion && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2, minmax(0, 1fr))", gap: 9 }}>
            {currentQuestion.choices?.map((choice) => (
              <button
                ...
                style={{ ...soft, minHeight: 60, padding: "13px 14px", color: textPrimary, cursor: "pointer", textAlign: "left", fontSize: 13.5, fontWeight: 950, boxShadow: "0 8px 22px rgba(30,45,60,0.05)" }}
              >
                {choice.label}
              </button>
            ))}
          ...
          <div style={{ ...soft, padding: 13 }}>
            <div style={{ color: textSecondary, fontSize: 11.5, fontWeight: 850, marginBottom: 8 }}>Oder eigene Antwort eingeben</div>
            <div style={{ display: "flex", gap: 8, flexDirection: isMobile ? "column" : "row" }}>
              <input
                value={freeText}
                onChange={(event) => setFreeText(event.target.value)}
                ...
                style={{ flex: 1, height: 46, borderRadius: 13, border: `1px solid rgba(${accentRgb}, 0.17)`, background: "rgba(255,255,255,0.82)", color: textPrimary, padding: "0 12px", outline: "none", fontSize: 13.5, minWidth: 0 }}
              />
              <button type="button" onClick={submitText} disabled={!freeText.trim()} style={{ ...primary, minHeight: 46, opacity: freeText.trim() ? 1 : 0.45 }}>
                Weiter
              </button>
            </div>
          </div>
      ...
      {phase !== "questions" && (
        <>
            ...
            {phase === "review" ? (
              <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
                {answeredQuestions.map((question) => (
                  <button key={question.id} type="button" onClick={() => editAnswer(question.id)} style={{ border: "1px solid rgba(20,45,65,0.08)", borderRadius: 13, background: "rgba(255,255,255,0.70)", color: textPrimary, padding: "11px 12px", cursor: "pointer", textAlign: "left" }}>
                    <span style={{ display: "block", color: textSecondary, fontSize: 10.5, fontWeight: 850 }}>{question.question}</span>
                    <span style={{ display: "block", marginTop: 3, fontSize: 13, fontWeight: 950 }}>{answers[question.id]} · ändern</span>
                  </button>
                ))}
              </div>
            ) : (
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 9 }}>
            {phase === "review" ? (
              <>
                <button type="button" onClick={resetFlow} style={{ ...secondary, minHeight: 49 }}>Neu beginnen</button>
                <button type="button" onClick={() => setPhase("success")} style={{ ...primary, minHeight: 49 }}>Empfehlung erstellen</button>
              </>
            ) : (
              <>
                <button type="button" onClick={() => { setPhase("review"); }} style={{ ...secondary, minHeight: 49 }}>Antworten ändern</button>
                <button type="button" onClick={() => onPanelChange("home")} style={{ ...secondary, minHeight: 49 }}>Zur Übersicht</button>
                <button type="button" onClick={resetFlow} style={{ ...primary, minHeight: 49 }}>Neue Beratung</button>
              </>
            )}
          </div>
        </>
...
function FsazFocusedDemo({
  ...
  const [goal, setGoal] = useState("Ruhig einsteigen");
  const [language, setLanguage] = useState("Deutsch");
  const [timeWish, setTimeWish] = useState("");
  const [studentPath, setStudentPath] = useState<"internal" | "external" | null>(null);
  const [showAppDemo, setShowAppDemo] = useState(false);
  ...
  const glass: CSSProperties = {
    ...
    boxShadow:
      "0 18px 54px rgba(25,55,76,0.10), inset 0 1px 0 rgba(255,255,255,0.78)",
    backdropFilter: "blur(22px) saturate(165%)",
    WebkitBackdropFilter: "blur(22px) saturate(165%)",
  };
  ...
  const soft: CSSProperties = {
    borderRadius: 19,
    border: `1px solid rgba(${accentRgb}, 0.15)`,
    background: `linear-gradient(145deg, rgba(${accentRgb}, 0.09), rgba(255,255,255,0.70))`,
  };
  ...
  const primary: CSSProperties = {
    ...
    fontWeight: 900,
    cursor: "pointer",
    boxShadow: `0 12px 30px rgba(${accentRgb}, 0.22)`,
  };
  ...
  const featureItems = [
    ["🖥️", "Großes Sichtfeld", "Drei Monitore für Kreuzungen, Abbiegen und Querverkehr."],
    ["🚘", "Echtes Fahrgefühl", "Cockpit, Pedalerie und Schaltung nah am Pkw."],
    ["🔁", "Wiederholen", "Schwierige Situationen ohne realen Verkehr mehrfach trainieren."],
    ["🎯", "Gezielt üben", "Grundausbildung, Überland, Autobahn und Automatik."],
  ];
  ...
  function requestTraining() {
    ...
    onAsk(
      `${typeText} Mein Trainingsziel ist "${goal}". Wunschsprache: ${language}. Zeitwunsch: ${timeWish || "noch offen"}. Bitte bereite mit mir den nächsten sinnvollen Schritt vor.`,
    );
    onPanelChange("home");
  }
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={() => onPanelChange("home")}
          style={{ ...secondary, minHeight: 38, padding: "0 12px", fontSize: 12.5 }}
        >
          ← Übersicht
        </button>
      ...
      {panel === "dashboard" && (
        <>
          <div style={{ padding: isMobile ? "6px 4px 2px" : "8px 6px 4px" }}>
            <div style={{ ...smallLabel, color: accent }}>SIMULATOR ENTDECKEN</div>
            <div style={{ fontSize: isMobile ? 27 : 34, fontWeight: 950, marginTop: 5 }}>
              Trainieren, bevor es auf der Straße stressig wird.
            </div>
            <div style={{ color: textSecondary, fontSize: 14, lineHeight: 1.55, marginTop: 8, maxWidth: 720 }}>
              Der Simulator ergänzt die praktische Fahrausbildung. Du kannst Abläufe,
              Blickführung und schwierige Situationen kontrolliert üben und wiederholen.
            </div>
          </div>
          ...
          >
            {featureItems.map(([icon, title, detail]) => (
              <div key={title} style={{ ...soft, padding: 15, display: "flex", gap: 12 }}>
                <div style={{ fontSize: 23 }}>{icon}</div>
                <div>
                  <div style={{ fontWeight: 950 }}>{title}</div>
                  <div style={{ color: textSecondary, fontSize: 12.5, lineHeight: 1.45, marginTop: 4 }}>
                    {detail}
                  </div>
                </div>
              </div>
          ...
          <div style={{ ...glass, padding: 16 }}>
            <div style={{ ...smallLabel }}>PREISORIENTIERUNG</div>
            <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
              {[
                ["Grundausbildung · 6 × 45 Min.", "180 €"],
                ["Komplettpaket · 9 × 45 Min.", "270 €"],
                ["Externe Fahrschüler · Anmeldung", "+ 30 €"],
              ].map(([label, value]) => (
                <div
                  key={label}
                  style={{
          ...
          >
            <button
              type="button"
              onClick={() => {
                setStudentPath("internal");
                onPanelChange("connect");
              }}
            ...
            <button
              type="button"
              onClick={() => {
                setStudentPath("external");
                onPanelChange("courses");
              }}
              style={{ ...primary, minHeight: 64, textAlign: "left" }}
            >
              <span style={{ display: "block", fontSize: 14, fontWeight: 950 }}>Ich bin bei einer anderen Fahrschule</span>
              <span style={{ display: "block", marginTop: 3, fontSize: 11.5, opacity: 0.85 }}>Trotzdem bei FSAZ trainieren</span>
            </button>
          </div>
        </>
      )}
      ...
      {panel === "connect" && (
        <>
          <div style={{ padding: isMobile ? "6px 4px 2px" : "8px 6px 4px" }}>
            <div style={{ ...smallLabel, color: accent }}>MEIN FSAZ</div>
            <div style={{ fontSize: isMobile ? 27 : 34, fontWeight: 950, marginTop: 5 }}>
          ...
          >
            <div style={{ ...soft, padding: 18 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
                <div>
                  <div style={{ ...smallLabel }}>APP-ZUGANG · DEMO</div>
                  <div style={{ fontSize: 20, fontWeight: 950, marginTop: 4 }}>
                    Persönlicher QR-Zugang
                  </div>
                </div>
                <div
                ...
                >
                  {qrPattern.map((filled, index) => (
                    <span
                      ...
                      style={{
                        borderRadius: 1,
                        background: filled ? "#1b2e3b" : "transparent",
                      }}
                    />
                  ))}
                </div>
              </div>
              <div style={{ color: textSecondary, fontSize: 12.5, lineHeight: 1.5, marginTop: 14 }}>
                Kein echter QR-Code in der Demo. Die Oberfläche zeigt nur, wie der
                App-Zugang später sichtbar eingebunden werden kann.
              </div>
              <button
                type="button"
                onClick={() => setShowAppDemo((value) => !value)}
                style={{ ...secondary, width: "100%", marginTop: 14 }}
              >
                {showAppDemo ? "Demo-Zugang schließen" : "Demo-Zugang ansehen"}
              </button>
            </div>

            <div style={{ ...glass, padding: 18 }}>
              <div style={{ ...smallLabel }}>TRAININGSPLAN · BEISPIEL</div>
              <div style={{ fontSize: 21, fontWeight: 950, marginTop: 4 }}>
                {showAppDemo ? "Alex · Grundausbildung" : "Persönlicher Trainingsstand"}
              </div>
              <div style={{ color: textSecondary, fontSize: 12.5, lineHeight: 1.5, marginTop: 7 }}>
                {showAppDemo
                  ? "2 von 6 Grundeinheiten abgeschlossen. Als Nächstes: Anfahren, Schalten und Blickführung."
                  : "Öffne den Demo-Zugang, um beispielhaft zu sehen, wie Fortschritt und nächstes Trainingsziel zusammenlaufen."}
              </div>

              <div style={{ marginTop: 16, height: 9, borderRadius: 999, background: `rgba(${accentRgb}, 0.10)`, overflow: "hidden" }}>
                <div
                  style={{
                    width: showAppDemo ? "33%" : "0%",
                    height: "100%",
                    background: accent,
                    borderRadius: 999,
                    transition: "width 220ms ease",
              ...
              <button
                ...
                onClick={() => {
                  setStudentPath("internal");
                  onPanelChange("schedule");
                }}
                style={{ ...primary, width: "100%", marginTop: 16 }}
              >
                Nächstes Training vorbereiten
              </button>
            </div>
          </div>
        </>
      ...
      {panel === "courses" && (
        <>
          <div style={{ ...soft, padding: isMobile ? 18 : 22 }}>
            <div style={{ ...smallLabel, color: accent }}>EXTERNE FAHRSCHÜLER</div>
            <div style={{ fontSize: isMobile ? 27 : 34, fontWeight: 950, marginTop: 5 }}>
              Du bleibst bei deiner Fahrschule.
            </div>
            <div style={{ color: textSecondary, fontSize: 14, lineHeight: 1.55, marginTop: 8 }}>
              Für das Simulatortraining ist kein Fahrschulwechsel nötig. Du kannst
              FSAZ gezielt als zusätzliches Training nutzen.
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 13 }}>
              {["Kein Fahrschulwechsel", "Eigene Fahrschule bleibt", "Externe Anmeldung: 30 €"].map((item) => (
                <span
                  key={item}
                  style={{
                    borderRadius: 999,
          ...
          <div style={{ ...glass, padding: 17 }}>
            <div style={{ fontWeight: 950 }}>Was möchtest du trainieren?</div>
            <div style={{ color: textSecondary, fontSize: 12.5, marginTop: 4 }}>
              Eine Auswahl reicht. Weitere Details kommen erst danach.
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 13 }}>
          ...
          >
            <div style={{ ...soft, padding: 17 }}>
              <div style={{ ...smallLabel }}>EMPFOHLENER EINSTIEG</div>
              <div style={{ fontSize: 20, fontWeight: 950, marginTop: 5 }}>
                {goal === "Ruhig einsteigen" || goal === "Schalten & Anfahren"
                  ? "Grundausbildung · 6 × 45 Min."
                  : `${goal} gezielt trainieren`}
              </div>
              <div style={{ color: textSecondary, fontSize: 12.5, lineHeight: 1.5, marginTop: 7 }}>
                {goal === "Ruhig einsteigen" || goal === "Schalten & Anfahren"
                  ? "Bedienung, Anfahren, Schalten und sichere Routinen Schritt für Schritt aufbauen."
                  : "Das Trainingsziel wird gezielt vorbereitet. Den konkreten Modulumfang bestätigt FSAZ vor der Anmeldung."}
              </div>
            </div>

            <div style={{ ...glass, padding: 17 }}>
              <div style={{ ...smallLabel }}>PREISE</div>
              <div style={{ display: "grid", gap: 8, marginTop: 8, fontSize: 13 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                  <span>Grundausbildung</span><strong>180 €</strong>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                  <span>Komplettpaket</span><strong>270 €</strong>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                  <span>Externe Anmeldung</span><strong>30 €</strong>
                </div>
              </div>
            </div>
          </div>
      ...
      {panel === "schedule" && (
        <>
          <div style={{ padding: isMobile ? "6px 4px 2px" : "8px 6px 4px" }}>
            <div style={{ ...smallLabel, color: accent }}>TRAINING ANFRAGEN</div>
            <div style={{ fontSize: isMobile ? 27 : 34, fontWeight: 950, marginTop: 5 }}>
              Drei Angaben. Dann ist die Anfrage vorbereitet.
            </div>
          </div>

          <div style={{ ...glass, padding: 17 }}>
            <div style={{ fontWeight: 950 }}>1 · Wo machst du deinen Führerschein?</div>
            <div
              style={{
                display: "grid",
                gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr",
            ...
            >
              <button
                type="button"
                onClick={() => setStudentPath("internal")}
                style={{
                  ...secondary,
                  minHeight: 58,
                  background:
              ...
              <button
                type="button"
                onClick={() => setStudentPath("external")}
                style={{
                  ...secondary,
                  minHeight: 58,
                  background:
          ...
          <div style={{ ...glass, padding: 17 }}>
            <div style={{ fontWeight: 950 }}>2 · Trainingsziel</div>
          ...
          >
            <label style={{ ...glass, padding: 16, display: "flex", flexDirection: "column", gap: 8 }}>
              <span style={{ fontWeight: 950 }}>3 · Wunschzeit</span>
              <input
                value={timeWish}
                onChange={(event) => setTimeWish(event.target.value)}
            ...
            <label style={{ ...glass, padding: 16, display: "flex", flexDirection: "column", gap: 8 }}>
              <span style={{ fontWeight: 950 }}>Sprache</span>
              <select
                value={language}
              ...
              >
                {["Deutsch", "Englisch", "Türkisch", "Arabisch", "Andere Sprache"].map((item) => (
          ...
          {studentPath === "external" && (
            <div style={{ ...soft, padding: 14, color: textSecondary, fontSize: 12.5, lineHeight: 1.5 }}>
              Du bleibst bei deiner bisherigen Fahrschule. Für externe Fahrschüler
              ist laut veröffentlichter Preisliste eine zusätzliche Anmeldung von 30 € vorgesehen.
            </div>
          )}
          ...
          <button
            ...
            disabled={!studentPath}
            style={{
              ...primary,
              width: "100%",
              opacity: studentPath ? 1 : 0.45,
              cursor: studentPath ? "pointer" : "not-allowed",
            }}
          >
            Anfrage im Chat vorbereiten
          </button>
        </>
      )}

      {(panel === "documents" || panel === "coach") && (
        <>
          <div style={{ ...soft, padding: isMobile ? 18 : 22 }}>
            <div style={{ ...smallLabel, color: accent }}>ZWEI KLARE WEGE</div>
            <div style={{ fontSize: isMobile ? 26 : 32, fontWeight: 950, marginTop: 5 }}>
              Entscheidend ist nur: Wo machst du deinen Führerschein?
            </div>
          </div>
          <div
            style={{
          ...
          >
            ...
            <button
              ...
              style={{ ...primary, minHeight: 76, textAlign: "left" }}
            >
              <strong>Andere Fahrschule</strong>
              <span style={{ display: "block", opacity: 0.86, fontSize: 11.5, marginTop: 5 }}>
                Ohne Wechsel zusätzlich am Simulator trainieren.
              </span>
            </button>
          </div>
        </>
